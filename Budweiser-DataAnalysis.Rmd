...
author: "Paul-Vijayasrikanth Kaniti"
date: "2/10/2020"
output:
...
## Data Analysis and presentation done by Paul Huggins and Vijayasrikanth Kaniti
...
# Load datasets into R studio supplied by Budweiser,Place Beers.csv and Breweries.csv in same folder as rmd files---
...
ggplot(accu,aes(x=k,y=accuracy)) +
  geom_line() + 
  labs(x="K",y="Accuracy")+geom_vline(xintercept =9,linetype="dotted",color="blue",size=1)+geom_hline(yintercept = 0.78,linetype="dotted",color="blue",size=1)
# The best value for k appears to be around 30. We will use k=8 going forward.

# KNN Test ---
KNNIPA <- knn(trainIpa[,1:2],testIpa[,1:2],cl=trainIpa$Style,k=9,prob = TRUE)
...
```
```{r}
#Group Beers based on ABv and check what are the beers that are most likeable.

KNNData9 <- MergeData_Imp

# Filter and Replace IPA & Ale ---

for (i in 1:2410) {
  if (KNNData9[i,7]>0.0857) {
    KNNData9[i,11] <- "High"
  } else if (KNNData9[i,7]>0.0433 & KNNData9[i,7]<0.0857 ) {
    KNNData9[i,11] <- "Medium" 
...
KNNData9$Group<-KNNData9$V11

KNNData9%>%ggplot(aes(x=ABV,fill=Group))+geom_histogram()
